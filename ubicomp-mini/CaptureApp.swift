...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        // Crash recovery: restore system screenshot shortcuts immediately in case prior run died uncleanly
        ...
        CaptureLog.info("Clipboard panel hotkey registered")
        ...
        // Browsing timeline: passive tab tracking + Chrome history import
        BrowsingTimelineService.shared.start()
        CaptureLog.info("Browsing timeline started")

        ChromeHistoryImporter.shared.start()
        CaptureLog.info("Chrome history importer started")
    ...
    func applicationWillTerminate(_ notification: Notification) {
        BrowsingTimelineService.shared.stop()
        ChromeHistoryImporter.shared.stop()
        FileMonitor.shared.stop()
        RecordingRegionBorder.shared.dismiss()
        RecordingToolbarController.shared.dismiss()
        ScreenRecordingCapture.shared.forceFinalize()
        ...
        ClipboardPanelController.shared.teardown()
