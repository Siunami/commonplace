...
final class DatabaseManager {
    ...
    func appFacets() -> [(appName: String, bundleId: String?, count: Int)] {
        ...
        return (try? dbQueue.read { db in
            let rows = try Row.fetchAll(db, sql: """
                SELECT sourceApp, bundleId, COUNT(*) as cnt
                FROM highlight
    ...
    func insertRecording(_ record: inout RecordingRecord) {
        guard let dbQueue else {
            CaptureLog.error("Cannot insert recording: database unavailable (degraded mode)")
            return
        }
        do {
            try dbQueue.write { db in
    ...
    func recordingByPath(_ filePath: String) -> RecordingRecord? {
        try? dbQueue?.read { db in
            try RecordingRecord
    ...
    func recordings(for dayString: String) -> [RecordingRecord] {
        (try? dbQueue?.read { db in
            try RecordingRecord
                .filter(Column("dayString") == dayString)
                .order(Column("timestamp").asc)
    ...
    func recentRecordings(limit: Int = 50) -> [RecordingRecord] {
        (try? dbQueue?.read { db in
            try RecordingRecord
                .order(Column("timestamp").desc)
                .limit(limit)
    ...
    func recordingDiskUsage() -> Int64 {
        let recordingsDir = ScreenRecordingCapture.recordingsBaseURL
        guard let enumerator = FileManager.default.enumerator(
    ...
    // MARK: - Saved Filters (Smart Folders)

    func allSavedFilters() -> [SavedFilter] {
        (try? dbQueue?.read { db in
            try SavedFilter
    ...
    func insertSavedFilter(_ filter: SavedFilter) {
        guard let dbQueue else { return }
        do {
            try dbQueue.write { db in try filter.insert(db) }
        } catch {
            CaptureLog.error("Failed to insert saved filter: \(error.localizedDescription)")
        }
    }
    ...
    func deleteSavedFilter(id: String) {
        ...
        do {
            try dbQueue.write { db in
                try db.execute(sql: "DELETE FROM saved_filter WHERE id = ?", arguments: [id])
            }
        } catch {
            CaptureLog.error("Failed to delete saved filter: \(error.localizedDescription)")
        }
    }

    func highlightsForFilter(_ filter: SavedFilter, offset: Int = 0, limit: Int = 200) -> [Highlight] {
        guard let dbQueue else { return [] }
        let (clause, args) = filter.toSQL()
        let sql = """
            SELECT * FROM highlight
            WHERE \(clause)
            ...
            """
        var allArgs = args
        allArgs.append(String(limit))
        allArgs.append(String(offset))
        return (try? dbQueue.read { db in
            try Highlight.fetchAll(db, sql: sql, arguments: StatementArguments(allArgs))
        }) ?? []
    }

    func countForFilter(_ filter: SavedFilter) -> Int {
        guard let dbQueue else { return 0 }
        let (clause, args) = filter.toSQL()
        let sql = "SELECT COUNT(*) FROM highlight WHERE \(clause)"
        return (try? dbQueue.read { db in
    ...
    func insertPageVisit(_ visit: inout PageVisit) {
        guard let dbQueue else { return }
        do {
            try dbQueue.write { db in try visit.insert(db) }
        } catch {
            CaptureLog.error("Failed to insert page visit: \(error.localizedDescription)")
        }
    }

    func closePageVisit(id: Int64, at timestamp: Double) {
        guard let dbQueue else { return }
        do {
            try dbQueue.write { db in
                try db.execute(sql: """
                    UPDATE page_visit SET endedAt = ?, duration = ? - startedAt
                    WHERE id = ? AND endedAt IS NULL
                    """, arguments: [timestamp, timestamp, id])
            }
        } catch {
            CaptureLog.error("Failed to close page visit: \(error.localizedDescription)")
        }
    }

    func closeAllOpenVisits(at timestamp: Double) {
        guard let dbQueue else { return }
        try? dbQueue.write { db in
            try db.execute(sql: """
                UPDATE page_visit SET endedAt = ?, duration = ? - startedAt
    ...
    func pageVisits(for dayString: String) -> [PageVisit] {
        guard let dbQueue else { return [] }
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd"
        guard let dayDate = formatter.date(from: dayString) else { return [] }
    ...
    func recentPageVisits(limit: Int = 100) -> [PageVisit] {
        (try? dbQueue?.read { db in
            try PageVisit
                .order(Column("startedAt").desc)
                .limit(limit)
    ...
    func searchPageVisits(query: String) -> [PageVisit] {
        let pattern = "%\(query)%"
        return (try? dbQueue?.read { db in
            try PageVisit.fetchAll(db, sql: """
    ...
    func incrementCaptureCount(forURL url: String, at timestamp: Double) {
        guard let dbQueue else { return }
        try? dbQueue.write { db in
            try db.execute(sql: """
                UPDATE page_visit SET captureCount = captureCount + 1
                WHERE url = ? AND startedAt <= ? AND (endedAt IS NULL OR endedAt >= ?)
                """, arguments: [url, timestamp, timestamp])
        }
    }

    func markBookmarkedURLs(_ urls: Set<String>) {
        guard let dbQueue, !urls.isEmpty else { return }
        try? dbQueue.write { db in
            // Reset all bookmarks first
            try db.execute(sql: "UPDATE page_visit SET isBookmarked = 0 WHERE isBookmarked = 1")
            // Mark matching URLs
            let placeholders = urls.map { _ in "?" }.joined(separator: ",")
            try db.execute(
                sql: "UPDATE page_visit SET isBookmarked = 1 WHERE url IN (\(placeholders))",
                arguments: StatementArguments(Array(urls))
            )
        }
    }

    func pageVisitExists(url: String, nearTimestamp: Double, tolerance: Double = 5.0) -> Bool {
        (try? dbQueue?.read { db in
            try PageVisit.fetchOne(db, sql: """
                SELECT * FROM page_visit
                WHERE url = ? AND ABS(startedAt - ?) < ?
                LIMIT 1
                """, arguments: [url, nearTimestamp, tolerance])
        }) != nil
    }
    ...
    func findOrCreateTag(name: String) -> Tag? {
        ...
        do {
            return try dbQueue.write { db in
                ...
                let tag = Tag(id: UUID().uuidString, name: trimmed, color: nil, createdAt: now, updatedAt: now)
                try tag.insert(db)
                return tag
            }
        } catch {
