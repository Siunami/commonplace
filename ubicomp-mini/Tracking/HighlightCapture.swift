...
final class HighlightCapture {
    ...
    func captureFromRecording(result: ScreenRecordingCapture.RecordingResult) {
        ...
        // Show toast with thumbnail — filePath is set so the toast's click-to-open works
        ...
        // Also reveal in Finder so the user knows where the file landed
    ...
    func captureFromUserScreenshot(filePath: String, image: NSImage, screenshotId: Int64?, context: CaptureContext, badgeLabel: String = "Screenshot") {
        ...
        let appName = context.sourceAppName ?? "Screenshot"
    ...
    private func saveHighlight(content: String, type: String, sourceApp: String?, entryId: String, screenshotId: Int64?, context: CaptureContext) -> String {
        ...
        // Correlate with browsing timeline: increment capture count on the active page visit
        if let url = context.sourceUrl, !url.isEmpty {
            db.incrementCaptureCount(forURL: url, at: highlight.timestamp)
        }
    ...
    func captureFromFileDetection(fileRecord: FileRecord, thumbnailImage: NSImage?) {
        let entryId = UUID().uuidString
